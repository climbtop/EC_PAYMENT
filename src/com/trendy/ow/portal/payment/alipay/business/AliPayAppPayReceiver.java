...
import com.trendy.ow.portal.payment.alipay.cache.AliPayAppCache;
...
public class AliPayAppPayReceiver extends PayReceiver {
	private static Logger log = LoggerFactory.getLogger(AliPayAppPayReceiver.class);
	...
	private PayItemBean processCallbackRequest(HttpServletRequest request) throws PortalServletException {
		...
		if (itemBean == null) {
			throw new PortalServletException(PayConfig.APP_ID, PayErrorCode.RECIVE_CALLBACK_EXCEPTION,
					"AlipayAppCallback received invalid callbackInfo:hava no itemId=" + payItemId + " record.");
		}
		...
		if (!checkCallbackInfo(paramMap, storeCode)) {
			throw new PortalServletException(PayConfig.APP_ID, PayErrorCode.RECIVE_CALLBACK_EXCEPTION,
					"AlipayAppCallback received invalid callbackInfo:sign fail.");
		}
	...
	public ChannelCallbackResultBean parseCallbackInfo(Map<String, String> paramMap) throws PortalServletException {
		log.info("AlipayAppCallback 收到支付信息：" + paramMap);
		String tradeStatus = paramMap.get("trade_status");
		if (!("TRADE_FINISHED".equals(tradeStatus) || "TRADE_SUCCESS".equals(tradeStatus))) {
			throw new PortalServletException(PayConfig.APP_ID, PayErrorCode.RECIVE_CALLBACK_EXCEPTION,
					"AlipayAppCallback received invalid callBackInfo:tradeStatus=" + tradeStatus);
		}
		...
		if (!StringKit.isValid(outTradeNo) || !StringKit.isValid(tradeNo) || !StringKit.isValid(sign)) {
			throw new PortalServletException(PayConfig.APP_ID, PayErrorCode.RECIVE_CALLBACK_EXCEPTION,
					"AlipayAppCallback received invalid callBackInfo:outTradeNo||tradeStatus||tradeNo||sign is empty.");
		}
		int itemId = 0;
		try {
			...
			throw new PortalServletException(PayConfig.APP_ID, PayErrorCode.RECIVE_CALLBACK_EXCEPTION,
					"AlipayAppCallback received invalid callBackInfo:parseInt outTradeNo fail.outTradeNo=" + outTradeNo);
		}
	...
	public boolean checkCallbackInfo(TreeMap<String, String> paramMap, String storeCode) throws PortalServletException {
		AliPayAppCache cache = new AliPayAppCache();
		AliPayAppConfigBean configBean = cache.getAliPayAppConfig(storeCode);
		return this.checkMsg(paramMap, configBean);
	}

	private boolean checkMsg(TreeMap<String, String> paramMap, AliPayAppConfigBean configBean) {
		...
		try {
			...
			if (!"true".equalsIgnoreCase(responseTxt)) {
				log.info("AlipayApp NotifyVerify -responseTxt=" + responseTxt + ";veryfy_url=" + veryfy_url);
				return false;
			}
		} catch (Exception e) {
			log.error("AlipayApp checkMsg 获取远程服务器ATN结果异常，veryfy_url=" + veryfy_url + "。", e);
			return false;
		}

		String markedSign = paramMap.get("sign");
		String preSignStr = AlipayUtil.getSignString(paramMap);
		log.info("preSignStr:" + preSignStr + ",TbRsaPublicKey:" + configBean.getTbRsaPublicKey());
		return RSASignature.doCheck(preSignStr, markedSign, configBean.getTbRsaPublicKey(), Constants.CODE_UNICODE);
	}
	...
	private PayItemBean processNotifyRequest(HttpServletRequest request) throws PortalServletException {
		...
		if (itemBean == null) {
			throw new PortalServletException(PayConfig.APP_ID, PayErrorCode.RECIVE_NOTIFY_EXCEPTION,
					"AlipayAppNotify received invalid notifyInfo:hava no itemId=" + payItemId + " record.");
		}
		...
		if (PayConfig.PAYS_PAYED.equalsIgnoreCase(itemBean.getPayStatus())
			...
			throw new PortalServletException(PayConfig.APP_ID, PayErrorCode.RECIVE_NOTIFY_EXCEPTION,
					"AlipayAppNotify Received repeated notifyInfo:itemId=" + payItemId + ",payStatus="
		...
		if (!checkNotifyInfo(paramMap, storeCode)) {
			throw new PortalServletException(PayConfig.APP_ID, PayErrorCode.RECIVE_NOTIFY_EXCEPTION,
					"AlipayAppNotify Received invalid notice:sign fail");
		}
		PayLogBusiness logBusiness = new PayLogBusiness();
	...
	public ChannelNotifyResultBean parseNotifyInfo(Map<String, String> paramMap) throws PortalServletException {
		log.info("AlipayAppNotifyReceiver 收到通知信息:" + paramMap.toString());
		String tradeStatus = paramMap.get("trade_status");
		if (!("TRADE_FINISHED".equals(tradeStatus) || "TRADE_SUCCESS".equals(tradeStatus))) {
			throw new PortalServletException(PayConfig.APP_ID, PayErrorCode.RECIVE_NOTIFY_EXCEPTION,
					"AlipayAppNotifyReceiver received notice,end process.tradeStatus=" + tradeStatus);
		}
		String outTradeNo = paramMap.get("out_trade_no");
		String tradeNo = paramMap.get("trade_no");
		String sign = paramMap.get("sign");
		...
		if (!StringKit.isValid(outTradeNo) || !StringKit.isValid(tradeStatus) || !StringKit.isValid(tradeNo)
				|| !StringKit.isValid(sign) || !StringKit.isValid(notifyId) || !StringKit.isValid(gmtPayment)
				|| !StringKit.isValid(totalFee) || !StringKit.isValid(signType)) {
			throw new PortalServletException(
					PayConfig.APP_ID,
					PayErrorCode.RECIVE_NOTIFY_EXCEPTION,
					"AlipayAppNotifyReceiver received invalid notifyInfo:outTradeNo||tradeStatus||tradeNo||sign||notifyId||gmtPayment||totalFee is empty.");
		}
		int itemId = 0;
		try {
			itemId = Integer.parseInt(outTradeNo);
			...
			throw new PortalServletException(PayConfig.APP_ID, PayErrorCode.RECIVE_NOTIFY_EXCEPTION,
					"AlipayAppNotifyReceiver received invalid notifyInfo:parseInt outTradeNo fail.outTradeNo="
							+ outTradeNo);
		}
